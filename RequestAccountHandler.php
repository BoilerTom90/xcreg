   ...
   $num_rows_affected = $p->Insert($id, $email, $school);
	if ($num_rows_affected > 0) {
      ...
      $alertCategory = "alert-success";
	   NotifyAdmin("Request Account Accepted for $email -- You will be notified shortly when account is approved."); 
	} else { 
      $status_msg = "Request not accepted. Account request previously submitted!";
      $alertCategory = "alert-warning";
	   NotifyAdmin("Request Account failed for $email -- There is already a request pending for this email. Please be patient!");
	}
